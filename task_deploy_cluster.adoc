...
The steps you follow depend on whether you want to create a single-node cluster or a multi-node cluster. A multi-node cluster can have two, four, six, eight, ten, or twelve nodes. 
...
After deploying the cluster, link:https://docs.netapp.com/us-en/ontap/system-admin/setup-autosupport-task.html[confirm the ONTAP Select AutoSupport feature is configured^] and then link:task_cli_clusters.html[back up the ONTAP Select Deploy configuration data].
