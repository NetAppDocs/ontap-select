...
+
You need to enable external key management and specify the external key management server. The external key management server serves authentication keys to nodes using the Key Management Interoperability Protocol (KMIP). For more information, refer to the link:https://docs.netapp.com/us-en/ontap/encryption-at-rest/configure-netapp-volume-encryption-concept.html[ONTAP NetApp volume and aggregate encryption documentation^].
