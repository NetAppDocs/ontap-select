...
. Select the figure icon at the top right of the page, then select *Change Password*.

. Provide the current and new password as prompted and select *Submit*.
...
. Select *Management Servers* > *Add vCenter*.

. Enter the following information and select *Add*.
+
...
. Optionally, select image:icon_kebab.gif[Options] after the new management server is added, then select one of the following:
+ 
* Update credentials
* Verify credentials
