...
Verify that ONTAP Select Deploy is not used to perform any other tasks during the upgrade. Refer to the link:reference_release_notes.html[release notes] for information and restrictions about upgrading the ONTAP Select Deploy utility.

[NOTE]
====
If you have an older instance of the ONTAP Select Deploy administration utility installed, you should upgrade to the current release. The ONTAP Select node and ONTAP Select Deploy component are upgraded independently. Refer to the link:concept_adm_upgrading_nodes.html[Upgrade the ONTAP Select nodes^] for further details.
