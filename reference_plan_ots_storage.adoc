...
When estimating the amount of storage for an HA pair, you must take into account that all aggregates (root and data) are mirrored. As a result, each plex of the aggregate consumes an equal amount of storage.
