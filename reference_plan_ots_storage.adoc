...
This is an LVM pool (storage pool) containing the system data used to host the ONTAP Select virtual machine. The LVM pool must be backed by a high endurance flash device, and can be either SAS or NVMe. An NVMe device is recommended for improved performance.
** Data disks
This is a set of SAS SSD drives used for data management. The SSD devices should be enterprise grade and durable. The NVMe interface is not supported.
