...
---
= ONTAP Select storage and RAID requirements
:hardbreaks:
:nofooter:
:icons: font
:linkattrs:
...
* Perform LUN configuration based on the following guidelines:
...
[NOTE]
If a hardware RAID controller is present, refer to the link:concept_stor_concepts_chars.html[Deep dive storage] section for additional configuration requirements.
...
There are also specific software package configuration requirements. Refer to the link:https://docs.netapp.com/us-en/ontap-select/kvm-host-configuration-and-preparation-checklist.html#prepare-linux-server[preparation of the Linux server] step for more information.
...
[TIP]
ONTAP Select enforces several restrictions related to storage allocation and use. Before you deploy an ONTAP Select cluster or purchase a license, you should be familiar with these restrictions. Refer to the link:concept_lic_evaluation.html[License] section for more information.
