...
+
SSeveral upgrade procedures are available. You should select the applicable procedure based on the following criteria:
...
+
Performing an ESXi upgrade using the Deploy administration utility is the more general and resilient option. However, there may be instances when Deploy is unavailable or cannot be used. For example, upgrading to ESXi 8.0 is not supported with earlier versions of ONTAP Select and the Deploy administration utility.
+
...
. Upgrade the hypervisor host where ONTAP Select is running to 0 or later using the procedure provided by VMware.
+
