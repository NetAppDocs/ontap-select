...
summary: If you are running ONTAP Select on VMware ESXi, you can upgrade the ESXi software from an earlier supported version to ESXi 8.0 or later. Before upgrading, you should understand the process and select the appropriate upgrade procedure.
---
= Upgrade to VMware ESXi 8.0 or later for ONTAP Select
:hardbreaks:
:nofooter:
:icons: font
:linkattrs:
...
[.lead]
If you are running ONTAP Select on VMware ESXi, you can upgrade the ESXi software from an earlier supported version to ESXi 8.0 or later. Before upgrading, you should understand the process and select the appropriate upgrade procedure.

== Prepare to upgrade VMware ESXi

Prepare and select the upgrade procedure that is appropriate for your environment before upgrading the ESXi software on the hypervisors hosting an ONTAP Select cluster.
...
+
Performing an ESXi upgrade using the Deploy administration utility is the more general and resilient option. However, there might be instances when Deploy is unavailable or cannot be used. For example, upgrading to ESXi 8.0 is not supported with earlier versions of ONTAP Select and the Deploy administration utility.

If you are using these earlier versions and attempt an upgrade, you can leave the ONTAP Select virtual machine in a state where it cannot be booted. In this case, you must select an upgrade procedure that does not use Deploy.

. *Upgrade the Deploy administration utility*
+
Before performing an upgrade procedure using the Deploy utility, you might need to upgrade your Deploy instance. In general, you should upgrade to the most recent version of Deploy. The Deploy utility must support the version of ONTAP Select you are using. Refer to the link:reference_release_notes.html[ONTAP Select release notes] for more information.
...
----
. Upgrade the hypervisor host where ONTAP Select is running to ESXi 8.0 or later using the procedure provided by VMware.
