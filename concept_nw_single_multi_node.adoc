...
summary: ONTAP Select supports both single node and multi-node network configurations.
---
= ONTAP Select single and multiple node network configurations
:hardbreaks:
:nofooter:
:icons: font
:linkattrs:
...
[.lead]
ONTAP Select supports both single-node and multi-node network configurations.
...
Unlike the multi-node version of the ONTAP Select product, each ONTAP Select VM contains three virtual network adapters, presented to ONTAP network ports e0a, e0b, and e0c.

These ports are used to provide the following services: management, data, and intercluster LIFs.

.KVM

You can deploy ONTAP Select as a single-node cluster. The hypervisor host includes a virtual switch that provides access to the external network.

.ESXi

The following figure shows the relationship between these ports and the underlying physical adapters. The figure depicts one ONTAP Select cluster node on the ESXi hypervisor.
...
As explained in the multi-node LIF assignment section of this document, ONTAP uses IPspaces to keep cluster network traffic separate from data and management traffic. The single-node variant of this platform does not contain a cluster network. Therefore, there are no ports present in the cluster IPspace.

[NOTE]
Cluster and node management LIFs are automatically created during ONTAP Select cluster setup. You can create the remaining LIFs post deployment.
...
The multi-node ONTAP Select network configuration consists of two networks.

These are an internal network, responsible for providing cluster and internal replication services, and an external network, responsible for providing data access and management services. End-to-end isolation of traffic that flows within these two networks is extremely important for building an environment that is suitable for cluster resiliency.

These networks are represented in the following figure, which shows a four-node ONTAP Select cluster running on a VMware vSphere platform. Six, eight, ten, and twelve-node clusters have a similar network layout.
...
*Overview of an ONTAP Select multi-node cluster network configuration*

image:DDN_01.jpg[Overview of an ONTAP Select multi-node cluster network configuration]
...
Ports e0a, e0b, and e0g reside on the external network. Although ports e0c through e0f perform several different functions, collectively they compose the internal Select network. When making network design decisions, you should place these ports on a single layer-2 network. There is no need to separate these virtual adapters across different networks.

The relationship between these ports and the underlying physical adapters is illustrated in the following figure, which depicts one ONTAP Select cluster node on the ESXi hypervisor.

*Network configuration of a single node that is part of a multi-node ONTAP Select cluster*
image:DDN_02.jpg[Network configuration of a single node that is part of a multi-node ONTAP Select cluster]

Segregating internal and external traffic across different physical NICs prevents insufficient access to network resources, introducing latencies into the system. Additionally, aggregation through NIC teaming allows the ONTAP Select cluster node to continue accessing the network if a single network adapter fails.

NOTE: Both the external network and internal network port groups contain all four NIC adapters in a symmetrical manner. The active ports in the external network port group are the standby ports in the internal network. Conversely, the active ports in the internal network port group are the standby ports in the external network port group.
...
[NOTE]
You cannot assign a static IP address to these interfaces, and you should not create additional cluster interfaces.

Cluster network traffic must flow through a low-latency, non-routed layer-2 network. Due to cluster throughput and latency requirements, you should physically locate the ONTAP Select cluster within proximity (for example, multipack, single data center). Building four, six, eight, ten, or twelve-node stretch cluster configurations by separating HA nodes across a WAN or significant geographical distances is not supported. A stretched two-node configuration with a mediator is supported.

For details, refer to the section link:reference_plan_best_practices.html#two-node-stretched-ha-metrocluster-sds-best-practices[Two-node stretched HA (MetroCluster SDS) best practices].

[NOTE]
To make sure of maximum throughput for cluster network traffic, this network port is configured to use jumbo frames (7500 to 9000 MTU). For correct cluster operation, verify that jumbo frames are enabled on all upstream virtual and physical switches providing internal network services to ONTAP Select cluster nodes.
...
[NOTE]
Port e0e is reserved by ONTAP for internal replication traffic. Therefore, neither the port nor the hosted LIF is visible in the ONTAP CLI or in System Manager. This interface is configured to use an automatically generated link local IP address, and you cannot assign an alternate IP address. This network port requires the use of jumbo frames (7500 to 9000 MTU).
...
[NOTE]
Like the RSM port (e0e), neither the physical port nor the hosted network interface is visible to users from either the ONTAP CLI or from System Manager. As a result, you cannot modify the IP address of this interface, and you cannot change the state of the port. This network port requires the use of jumbo frames (7500 to 9000 MTU).

// 2025 JAN 29, NADIAA updates
