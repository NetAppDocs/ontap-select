...
[.lead]
You can deploy a 90-day evaluation instance of an ONTAP select cluster ranging in size from a single-node up to 99 nodes using an OVF template on VMware ESXi. You can download an evaluation license if you are an existing or new customer. For example, you are an existing customer and you want to try a new configuration before purchasing a license.
...
For additional information about host configuration options, refer to the link:esxi-host-configuration-and-preparation-checklist.html[ONTAP Select Install documentation].
...
** Select *Datacenter*. Then right-select and select *Deploy OVF Template*.
