...
summary: You can deploy a 90-day evaluation instance of a single-node ONTAP Select cluster.
---
...
[.lead]
You can deploy a 90-day evaluation instance of an ONTAP select cluster ranging in size from a single-node up to 99 nodes. Currently, two hypervisors are qualified to work with ONTAP Select, VMware ESXi and RedHat Enterprise Linux (RHEL) and Rocky Linux based KVM. You can download an evaluation license if you are an existing or new customer. For example, you are an existing customer and you want to try a new configuration before purchasing a license.
...
* You must install the OVF template (contained in an ova file) using the vSphere standalone client or vSphere web client (the only option for ESXi 8.0 and later). You don't use the ONTAP Select Deploy administration utility.

NOTE: The following steps are for downloading an ONTAP evaluation license on an ESXi host. If you need information about downloading an evaluation license on a KVM host, contact your NetApp sales team.
