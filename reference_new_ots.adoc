...
+
To use software RAID for locally attached NVMe disks, you need to link:reference_chk_host_prep.html[configure PCI passthrough (DirectPath IO)] when you are preparing the KVM host. This provides direct access for the KVM host to the locally attached NVMe disks, which is required for the following tasks: 
+
...
ONTAP Select 9.17.1 includes support for a Kernel-Based Virtual Machine (KVM) hypervisor on Red Hat Enterprise Linux (RHEL) 9.6 and Rocky Linux 9.6.
