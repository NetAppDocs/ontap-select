...
Software RAID support for local attached storage on KVM host::
Beginning with ONTAP Select 9.17.1, software RAID is supported for ONTAP Select locally attached NVMe disks on a KVM hypervisor host.
+
To use software RAID for locally attached NVMe disks, you need to link:https://docs.netapp.com/us-en/ontap-select/reference_chk_host_prep.html[configure PCI passthrough (DirectPath IO)] when you are preparing the KVM host. This provides direct access for the KVM host to the locally attached NVMe disks, which is required to use software RAID after you deploy the cluster.
+
link:https://docs.netapp.com/us-en/ontap-select/concept_stor_swraid_local.html[Learn about the software RAID configuration services for local attached storage].
...
Beginning with ONTAP Select 9.17.1, the FreBSD NDA (NVMe Direct Access) device driver replaces the NVD (NVMe express disk driver). The FreBSD driver provides support for direct access devices by implementing the NVMe command protocol. When you deploy an ONTAP Select 9.17.1 cluster or upgrade an existing ONTAP Select cluster to version 9.17.1, the FreBSD driver is started by default when you configure the NVMe devices. 
+
...
Beginning with ONTAP Select 9.16.1, KVM hypervisor is supported on Red Hat Enterprise Linux (RHEL) 9.5 and Rocky Linux 9.5.
