...
Beginning with ONTAP Select 9.17.1, support for vSAN Express Storage Architecture (ESA) has been extended to deploy multi node clusters. vSAN Express Storage Architecture is an ESX specific configuration, a new architecture for VMware vSAN introduced in vSphere 8.
+
...
+
vSAN Express Storage Architecture is designed to offer enhanced efficiency, scalability, and performance, particularly when using NVMe-based TLC flash devices.
+
This enhancement enables customers to deploy vSAN Express Storage Architecture configurations.
+
[NOTE] 
====
