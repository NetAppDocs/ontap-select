...
+ 
When you are preparing the KVM host, you need to link:reference_chk_host_prep.html[configure PCI passthrough (DirectPath IO)] to provide direct access to the locally attached NVMe disks. The direct access is required to use software RAID after you deploy the ONTAP Select cluster on the KVM host.
+ 
