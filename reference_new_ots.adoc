...
Beginning with ONTAP Select 9.17.1, new deployments have SnapLock Select Enterprise (including tamperproof snapshot locking) licensed automatically. For upgrades from ONTAP Select 9.16.1 and earlier, you can download the Enterprise version of the SnapLock Select license (SnapLock Compliance is not supported on ONTAP Select) free of charge from the link:https://mysupport.netapp.com/site/[NetApp Support Site^] and apply it manually.
