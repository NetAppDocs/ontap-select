...
Support for external key management (KMIP)::
Beginning with ONTAP Select 9.19.1, ONTAP Select supports external key management using the Key Management Interoperability Protocol (KMIP). You can enable external key management after deploying an ONTAP Select 9.19.1 cluster or upgrading an existing cluster to ONTAP Select 9.19.1. For more information, refer to link:reference_lic_ontap_features.html#licensed-ontap-features[ONTAP features licensed by default].
+
ONTAP Select does not support cloud key managers.

Support for Advanced Micro Devices (AMD) processors::
Beginning with ONTAP Select 9.19.1, ONTAP Select supports AMD processors on Kernel-Based Virtual Machine (KVM) and ESXi hypervisor hosts. For more information, refer to link:reference_plan_ots_hardware.html[VMware hypervisor and hardware considerations].

== ONTAP Select 9.18.1

Updates to the ONTAP Select product download options::
The ONTAP Select product download options on the NetApp Support Site are changing with the ONTAP Select 9.18.1 release to *ONTAP Select Deploy* and *ONTAP Select Image*. These two new product download options replace the previous four options, as shown in the following table:
+
[cols="30,30,40",options="header"]
|===
|Previous product download option |New product download option |For more information, go to....
|ONTAP Select 
|ONTAP Select Deploy
...
|ONTAP Select Deploy
|link:task_cli_upgrade_deploy.html[Upgrade an ONTAP Select Deploy instance]
|ONTAP Select Node Upgrade
|ONTAP Select Image
|link:concept_adm_upgrading_nodes.html[Upgrade ONTAP Select nodes]
|===

Enhanced support for cluster expansion and contraction::
Beginning with ONTAP Select 9.18.1, cluster expansion and contraction is supported between four-node and twelve-node ONTAP Select clusters on ESXi and KVM hypervisor hosts.
+
--
You can increase the cluster size for an existing cluster on an ESXi or KVM host in the following increments:

* From four nodes to six, eight, ten, or twelve nodes
* From six nodes to eight, ten, or twelve nodes
* From eight nodes to ten or twelve nodes
* From ten to twelve nodes 

You can decrease the cluster size for an existing cluster on an ESXi or KVM host in the following increments:

* From twelve nodes to ten, eight, six, or four nodes
* From ten nodes to eight, six, or four nodes
* From eight to six or four nodes
* From six to four nodes
--
+
...
Beginning with ONTAP Select 9.18.1, KVM hypervisor is supported on Rocky Linux 10.1.

== ONTAP Select 9.17.1

Software RAID support for locally attached NVMe disks on KVM hosts::
ONTAP Select 9.17.1 introduces software RAID support for ONTAP Select locally attached NVMe disks on a KVM hypervisor host. 
+
...
Beginning with ONTAP Select 9.17.1, cluster expansion and contraction is supported between six-node and twelve-node clusters on ESXi hosts:
+
--
...
+
vSAN ESA is an ESXi specific configuration and is a new architecture for VMware vSAN introduced in vSphere 8. vSAN ESA is designed to offer enhanced efficiency, scalability, and performance, particularly when using NVMe-based TLC flash devices.
+
ONTAP Select Deploy and ONTAP Select for ESXi support the configuration of an ONTAP Select single-node and multi-node cluster, using either a vSAN or an external array type of datastore for its storage pool.
+
[NOTE] 
====
To perform Content Library operations, you need access to additional permission levels. This access is required for vSAN ESA deployments if you are working with restricted roles.

link:https://docs.netapp.com/us-en/ontap-select/reference_plan_dep_vmware.html#vmware-vcenter-server-considerations[Learn about accessing permission levels for Content Library operations^].
====
...
Beginning with ONTAP Select 9.16.1, the NetApp License File (NLF) support is updated. The new NLF format includes licenses for the ARP, ONTAP S3, and S3 SnapMirror features. link:reference_lic_ontap_features.html#licensed-ontap-features[Learn more].
+
...
NOTE: Support for cluster expansion and contraction is limited to ESXi clusters only.
...
If you are running a version of ONTAP Select Deploy 9.14.1 lower than 9.14.1P2, you should upgrade to ONTAP Select Deploy 9.14.1P2 as soon as possible. For more information, refer to the link:https://library.netapp.com/ecm/ecm_download_file/ECMLP2886733[ONTAP Select 9.14.1 Release Notes^].
...
Updated ONTAP Select Deploy support::
As of April 2024, ONTAP Select Deploy 9.13.1 is no longer available on the NetApp Support Site. If you are running ONTAP Select Deploy 9.13.1, you should upgrade to ONTAP Select Deploy 9.14.1P2 as soon as possible. For more information, refer to the link:https://library.netapp.com/ecm/ecm_download_file/ECMLP2886733[ONTAP Select 9.14.1 Release Notes^].
...
As of April 2024, ONTAP Select Deploy 9.12.1 is no longer available on the NetApp Support Site. If you are running ONTAP Select Deploy 9.12.1, you should upgrade to ONTAP Select Deploy 9.14.1P2 as soon as possible. For more information, refer to the link:https://library.netapp.com/ecm/ecm_download_file/ECMLP2886733[ONTAP Select 9.14.1 Release Notes^].
...
+
NetApp strongly recommends that customers plan and execute a full data migration from ONTAP Select for KVM to any other ONTAP platform, including ONTAP Select for ESXi. For more information, refer to the https://mysupport.netapp.com/info/communications/ECMLP2877451.html[EOA Notice^]
