...
CLuster expansion::
ONTAP Select 9.13.1 introduces cluster expansion to increase performance and storage capacity available in the cluster. Beginning with ONTAP Select 9.13.1, you can expand to a two node cluster by configuring both nodes as a HA pair. The procedure is automated and initiated from ONTAP Select Deploy using REST API, CLI, or the Web interface. To perform this procedure, you must meet certain hardware and software requirements. link:concept_cluster_expansion.html[Learn more].
