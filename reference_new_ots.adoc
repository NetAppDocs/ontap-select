...
Beginning with ONTAP Select 9.17.1, support for vSAN Express Storage Architecture (ESA) has been extended to deploy multi node clusters. This enhancement enables customers to deploy vSAN Express Storage Architecture configurations. 
+
vSAN Express Storage Architecture is an ESX specific configuration, and is a new architecture for VMware vSAN which has been introduced in vSphere 8. vSAN Express Storage Architecture is designed to offer enhanced efficiency, scalability, and performance, particularly when using NVMe-based TLC flash devices.
+
...
[NOTE] 
====
To perform content library operations, you'll need access to additional permission levels. This is required for vSAN Express Storage Architecture deployments if you are working with restricted roles.

link:https://docs.netapp.com/us-en/ontap-select/reference_plan_ots_vcenter.html#administrative-privileges[Learn about accessing permission levels^].
====
