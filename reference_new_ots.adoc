...
Support for 12 node cluster expansion and contraction on ESXi hosts::
Beginning with ONTAP Select 9.17.1, customers can now expand or contract clusters between 6 and 12 nodes when running on ESXi hypervisor hosts. This means you can increase the size of an existing ONTAP Select cluster from 6 nodes up to 12 nodes, or reduce it from 12 nodes down to as few as 6 nodes.
+
Currently, the capability to expand up to 12 nodes is limited to ESXi hosts only. For clusters running on KVM hypervisor hosts, the maximum supported size remains 8 nodes.
+
This enhancement provides greater flexibility for scaling ONTAP Select clusters on ESXi, while KVM support remains unchanged at this time.
+
.Updated Supported Expansion and Contraction Combinations on ESXi hosts:
+
* Expansion: 
+
You can expand clusters in the following increments: from 6 to 8 nodes, 6 to 10 nodes, 6 to 12 nodes, 8 to 10 nodes, 8 to 12 nodes, and 10 to 12 nodes.
+
* Contraction: 
+
You can contract clusters as follows: from 12 to 10 nodes, 12 to 8 nodes, 12 to 6 nodes, 10 to 8 nodes, 10 to 6 nodes, and 8 to 6 nodes.
