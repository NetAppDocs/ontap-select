...
Beginning with ONTAP Select 9.17.1, customers can now expand or contract clusters between a six-node and a twelve-node cluster when running on ESXi hypervisor hosts. This means you can increase the size of an existing ONTAP Select cluster from a six-node and a twelve-node cluster, or reduce it from a twelve-node cluster down to as few as a six-node cluster.
+
