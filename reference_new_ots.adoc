...
+
This enhancement enables customers on deploying vSAN Express Storage Architecture configurations.
+
NOTE: There is extra permission level we need to include "Content library administrator(sample)" to have API user account to perform content library operations. This has to be included in permissions required for vSAN Express Storage Architecture deployments if customer is working with restricted roles.
