...
Beginning with ONTAP Select 9.19.1, external key management using the Key Management Interoperability Protocol (KMIP) is supported. You can enable external key management after installing a new ONTAP Select 9.19.1 Deploy or upgrading to ONTAP Select Deploy 9.19.1 from a previous release. For more information, see link:reference_lic_ontap_features.html#licensed-ontap-features[ONTAP features licensed by default].
+
ONTAP Select does not support cloud key managers and Onboard Key Manager.
