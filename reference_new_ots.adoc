...
Beginning with ONTAP Select 9.17.1, you can expand a six-node cluster or contract a twelve-node cluster on ESXi hypervisor hosts. This enhancement provides greater flexibility for scaling ONTAP Select clusters on ESXi hosts. KVM support remains unchanged.
+
You can increase the size of an existing ONTAP Select cluster from six-nodes up to a maximum of twelve-nodes. You can reduce the size of an existing ONTAP Select cluster from twelve-nodes down to a minimum of six-nodes.
+
