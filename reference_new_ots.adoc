...
Support for cluster expansion and contraction for KVM::
Beginning with ONTAP Select 9.17.1, the cluster expansion and contraction feature is supported for Kernel-Based Virtual Machine (KVM) and ESXi hypervisor hosts. For ONTAP Select 9.16.1 and 9.15.1, cluster expansion and contraction is only supported for ESXi hypervisor hosts.
+
You can use the cluster expansion and contraction feature to increase the cluster size from a six-node to an eight-node cluster and decrease the cluster size from an eight-node to a six-node cluster. link:task_cluster_expansion_contraction.html[Learn more].
