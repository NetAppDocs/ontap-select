...
Beginning with ONTAP Select 9.17.1, SnapMirror cloud is supported with ONTAP Select. SnapMirror cloud is not licensed by default for ONTAP Select. When you deploy an ONTAP Select 9.17.1 cluster or upgrade an existing ONTAP Select cluster to version 9.17.1, you can install a SnapMirror cloud evaluation license. For more information, refer to link:reference_lic_ontap_features.html#other-ontap-features[ONTAP features that are separately licensed].
