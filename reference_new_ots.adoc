...
Beginning with ONTAP Select 9.17.1, customers can now expand or contract clusters between six-node and twelve-node clusters when running on ESXi hypervisor hosts. Users can increase the size of an existing ONTAP Select cluster from a six-node and a twelve-node cluster, or reduce it from a twelve-node cluster down to as few as a six-node cluster.
+
