...
+
You can use the cluster expansion and contraction feature to increase the cluster size from a six-node to an eight-node cluster and decrease the cluster size from an eight-node to a six-node cluster. link:task_cluster_expansion_contraction.html#contract-the-cluster[Learn more].
