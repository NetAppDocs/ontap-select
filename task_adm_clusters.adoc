...
. Select image:icon_kebab.gif[Options] on the right of the cluster and select *Take Offline*.
+
If the offline option is not available, the cluster is already in the offline state.

. Select *Yes* in the popup window to confirm the request.

. Select *Refresh* occasionally to confirm the cluster is offline.

. To bring the cluster back online, select image:icon_kebab.gif[Options] and select *Bring Online*.

. Select *Refresh* occasionally to confirm the cluster is online.
...
. Select image:icon_kebab.gif[Options] on the right of the cluster and select *Delete*.
+
If the delete option is not available, then the cluster is not in an offline state.

. Select *Refresh* occasionally to confirm the cluster is removed from the list.
...
[NOTE]
Instead of using the Deploy UI, you can use the cluster refresh command in the Deploy CLI shell to refresh a cluster.
...
. Select the *Clusters* tab at the top left of the page and select the desired cluster from the list.

. Select image:icon_kebab.gif[Options] on the right side of the page and select *Cluster Refresh*.
