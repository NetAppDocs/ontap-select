...
NetApp recommends using ESXi 8.0 or later and an NVMe disk for the datastore hosting the system disks. This configuration provides the best performance for the NVRAM partition.

[NOTE]
When installing on ESXi 8.0 or later, ONTAP Select uses the vNVMEe driver regardless of whether the system disk resides on an SSD or on an NVMe disk. This sets the VM hardware level to 13, which is compatible with ESXi 8.0 and later.
...
To learn more about how to configure an Open vSwitch on a hypervisor host, refer to the link:https://www.netapp.com/media/13134-tr4613.pdf[ONTAP Select on KVM Product Architecture and Best Practices^] technical report.
...
It is a best practice to regularly back up the Deploy configuration data, including after creating a cluster. This becomes particularly important with two-node clusters, because the mediator configuration data is included with the backup.

After creating or deploying a cluster, you should link:task_cli_clusters.html[back up the ONTAP Select Deploy configuration data].
...
NetApp recommends that the LACP mode be active on both the ESXi and the physical switches. Furthermore, the LACP timer should be set to fast (1 second) on the physical switch, ports, port channel interfaces, and on the VMNICs.
