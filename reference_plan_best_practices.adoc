...
NetApp recommends using ESX 8.0 or later and an NVMe disk for the datastore hosting the system disks. This configuration provides the best performance for the NVRAM partition.

[NOTE]
When installing on ESX 8.0 or later, ONTAP Select uses the vNVMEe driver regardless of whether the system disk resides on an SSD or on an NVMe disk. This sets the VM hardware level to 13, which is compatible with ESX 8.0 and later.
