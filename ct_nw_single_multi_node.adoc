---
sidebar: sidebar
permalink: ct_nw_single_multi_config.html
keywords: ontap select, single node network configuration, multinode network configuration
summary: single node and multinode network configurations
---
...
For details, see the section link:rt_plan_best_practices.html#two-node-stretched-ha-metrocluster-sds-best-practices[Two-node stretched HA (MetroCluster SDS) best practices]."
