...
---
= VMware ESXi external storage requirements for ONTAP Select
:hardbreaks:
:nofooter:
:icons: font
:linkattrs:
...
link:https://mysupport.netapp.com/matrix/[Interoperability Matrix Tool^]. In addition, ONTAP Select supports all external storage arrays described in the VMware Storage/SAN Compatibility documentation, including iSCSI, NAS (NFSv3), Fibre Channel, and Fibre Channel over Ethernet. External array support is limited by the ESXi version supported by ONTAP Select.
...
Review the VMware documentation and follow the applicable best practices identified for ESXi hosts. In addition:
...
* Verify that all generic external storage arrays use the available redundancy and HA features where possible
