...
[.lead]
You must acquire a license file for each ONTAP Select node running with a capacity tier license. The license file defines the storage capacity for the node and is locked to the node through the unique nine-digit serial number assigned by NetApp.
