...
---
= Confirm connectivity among the ONTAP Select nodes
:hardbreaks:
:nofooter:
:icons: font
:linkattrs:
...
. Select the *Administration* tab at the top of the page, then select *Network Checker*.

. Select *Start New Run*, then select the hosts and networks for the HA pair
+
You can add and configure additional HA pairs as needed.

. Select *Start* to begin the network connectivity test.
