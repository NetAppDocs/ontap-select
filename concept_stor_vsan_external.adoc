...
The minimum requirement is that the underlying configuration is supported by the hypervisor that you are using (VMware ESXi or KVM on a supported Linux host). If the hypervisor is ESXi, it should be listed on the respective VMware HCLs.
