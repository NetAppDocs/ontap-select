...
This procedure is designed to be performed before creating a new ONTAP Select cluster. You can also perform the procedure to configure NVMe drives for existing cluster. In this case, after configuring the drives, you must add them through Deploy as you would additional SSD drives. The main difference is that Deploy detects the NVMe drives and reboots the nodes. Note the following:
