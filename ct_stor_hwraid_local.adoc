...
[NOTE]
Although support for Storage vMotion provides a lot of flexibility, it is important that the new host can appropriately support the ONTAP Select node. If a RAID controller and DAS storage were used on the original host, a similar setup should exist on the new host. Severe performance issues can result if the ONTAP Select VM is rehosted on an unsuitable environment.

//David, do you have the information in the steps below? I am going to remove it here, but feel free to take it if you need it.
...
[NOTE]
Adding capacity to an ONTAP Select VM likely results in VMDKs of different sizes. For details, see the section link:ct_stor_capacity_inc.html[Increasing storage capacity]. Unlike FAS systems, different sized VMDKs can exist in the same aggregate. ONTAP Select uses a RAID 0 stripe across these VMDKs, which results in the ability to fully use all the space in each VMDK regardless of its size.
