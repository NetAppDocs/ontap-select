...
---
= ONTAP Select requirements and planning
:hardbreaks:
:nofooter:
:icons: font
:linkattrs:
...
For more information on preparing your Linux server, including the required configuration and software packages, refer to the link:kvm-host-configuration-and-preparation-checklist.html[host configuration checklist]. Refer to the hypervisor requirements for the currently supported Linux distributions.
...
An ONTAP Select cluster is composed of one, two, four, six, eight, ten, or twelve nodes. You should determine the size of the cluster based on the application requirements. For example, if HA capability is needed for an enterprise deployment, then a multi-node cluster should be used.
...
Refer to the hypervisor-specific planning information and link:reference_release_notes.html[release notes] for additional details regarding the supported platforms.
