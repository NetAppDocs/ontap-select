...
---
= ONTAP Select Deploy hypervisor host requirements
:hardbreaks:
:nofooter:
:icons: font
:linkattrs:
...
NOTE: Refer to the hypervisor-specific planning information and link:reference_release_notes.html[release notes] for additional details regarding the supported platforms.
...
* A Deploy utility running on VMware ESXi can create and manage ONTAP Select clusters on either VMware ESXi or KVM.
* A Deploy utility running on KVM can create and manage ONTAP Select clusters on either VMware ESXi or KVM.
...
Your ESXi hypervisor host must meet several minimum resource requirements. Check that the hosts where ONTAP Select is deployed meet the following basic requirements:
...
Check that the ONTAP Select Deploy virtual machine network interface is configured and has a single management IP address. You can use DHCP to dynamically assign an IP address or manually configure a static IP address.
...
Deploy uses the VMware VIX API to communicate with the vCenter server and ESXi hosts. Initially, it establishes a connection using SOAP over SSL on TCP port 443. After this, a connection is opened using SSL on port 902. In addition, Deploy issues PING commands to verify that there is an ESXi host at the IP address you specify.

Additionally, Deploy must be able to communicate with the ONTAP Select node and cluster management IP addresses using the following protocols:
...
.VMware vSphere privileges for ESA

To create and manage vSAN Express Storage Architecture (ESA) in vCenter, you need the following vSphere Content Library privileges:

NOTE: To access the Content Library privileges, an administrator needs to grant you access to Global Permissions in vCenter Server.
...
// 2026 JUN 24, ON
// 2025 OCT 30, ONTAPDOC-3457 9.18.1RC
