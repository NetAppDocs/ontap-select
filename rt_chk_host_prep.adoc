...
summary: prepare hypervisor hosts for ontap select cluster deployment
---
...
IMPORTANT: The ONTAP Select Deploy administration utility does not perform the required network and storage configuration of the hypervisor hosts. You must manually prepare each host prior to deploying an ONTAP Select cluster.
