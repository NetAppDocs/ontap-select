...
Download the ONTAP Select Deploy package from the NetApp support site.
...
.About this task
The ONTAP Select Deploy administration utility is packaged as a virtual machine (VM) based on the Open Virtualization Format (OVF) standard. The single compressed file has the suffix `OVA` for ESX hosts and `TGZ` for KVM hosts. The VM provides the Deploy server and installation images for ONTAP Select nodes.
...
. Select your desired ONTAP Select version.
...
Install and start the ONTAP Select Deploy VM using the OVF VM image. During the installation, you configure the network interface to use DHCP or a static IP configuration.
...
. After the VM is deployed, select the new VM. If it isn't powered on based on your input to the deployment wizard, power it on manually.
...
Sign in to the web user interface to confirm that the Deploy utility is available and set up the initial configuration.
