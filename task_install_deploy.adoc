...
Before verifying the ONTAP Select Deploy OVA signature, your system must meet the following requirements:
...
The following files should be obtained from the NetApp support site product download page:
...
| `ONTAPdeploy.ova`
| The product installation executable for ONTAP Select 9.13.1.
| `ONTAPdeploy.ova.sig`
...
| `ONTAPdeploy.ova.sig.tsr`
| The revocation request used by OCSP for the installer’s signature.
| `tsa-prod-ONTAP-Select-Deploy.pem`
| The public certificate for the TSR file.
| `tsa-prod-chain-ONTAP-Select-Deploy.pem`
| The public certificate CA Chain for the TSR file.
|===

Verify the `ONTAPdeploy.ova.sig` and `ONTAPdeploy.ova.sig.tsr` files using the associated certificates and validation commands.
...
* Verify the timestamp with the following command: 
+
----
openssl ts -verify -data ONTAPdeploy.ova.sig -in ONTAPdeploy.ova.sig.tsr -CAfile tsa-prod-chain-ONTAP-Select-Deploy.pem  -untrusted tsa-prod-chain-ONTAP-Select-Deploy.pem
----
