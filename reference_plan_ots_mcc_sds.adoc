...
MetroCluster SDS can be deployed on the same VMware ESXi and KVM hypervisors as supported for a two-node cluster in a ROBO environment. Note that KVM is no longer supported with ONTAP Select 9.10.1 and later releases.
