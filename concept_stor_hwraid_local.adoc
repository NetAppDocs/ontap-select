...
NOTE: You cannot detach virtual disks from ONTAP Select VMs that are using the hardware RAID configuration. Detaching virtual disks is only supported for ONTAP Select VMs that are using the software RAID configuration. See link:task_adm_replace_drives_swraid.html[Replace a failed drive when using SW RAID] for more information.
