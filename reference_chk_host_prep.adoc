...
[.lead]
Prepare each of the hypervisor hosts where an ONTAP Select node is deployed. As part of preparing the hosts, carefully assess the deployment environment to make sure that the hosts are correctly configured and ready to support the deployment of an ONTAP Select cluster.
...
== Prepare a KVM hypervisor host

Prepare the KVM hypervisor host.
...
+
You need to configure PCI passthrough (DirectPath IO) to provide direct access to the locally attached storage NVMe discs on the KVM hypervisor host.
+
...
== Prepare an ESXi hypervisor host
...
== Configure Open vSwitch on the host 
Configure Open vSwitch on the KVM or ESXi hypervisor host.
...
The following steps are for a vSwitch configuration for an ESXi host with two physical ports (2x10Gb). 
