...
---
= Manage ONTAP Select diagnostics and support
:hardbreaks:
:nofooter:
:icons: font
:linkattrs:
...
. Select *Settings & AutoSupport*, then select image:icon_pencil.gif[Edit].

. Provide the configuration data as appropriate for your environment and select *Modify*.
+
...
. Select *Events & Jobs*, then select *Events*.

. Optionally, select *Filter* and create a filter to limit the event messages displayed.
...
. Select *Settings & AutoSupport* and then select image:icon_kebab.gif[Options].
...
. Optionally, select the correct package under *AutoSupport History*, then select the download icon to save the AutoSupport file to your local workstation.
