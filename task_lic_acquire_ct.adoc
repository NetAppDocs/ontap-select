...
.Before you begin
* You must have the nine-digit license serial number assigned to the ONTAP Select node by NetApp.
* You should wait at least 24 hours after the shipped date of your purchase order before acquiring a license file.
* You have reviewed the link:sizing-capacity-tier-license.html[considerations for sizing a Capacity Tiers license]. 

.About this task
You must perform this task for each ONTAP Select node requiring a Capacity Tiers license.

.Steps

. link:https://register.netapp.com/register/getlicensefile[Access the ONTAP Select license site^].
...
. Fill in the remaining fields on the *License Generator* page, including the *Product Serial #*.
+
The "product serial #" is the serial number for the ONTAP Select node.

. Select *Submit*.

. After the request has been validated, select the delivery method for the license.
+
You can select either *Download License* or *Email License*.
