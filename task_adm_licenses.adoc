...
[NOTE]
If an ONTAP Select node is deployed with an evaluation license, you can expand the storage up to the limit imposed by the license.
...
The twenty-digit node serial numbers originally assigned to each node for the evaluation deployment will be replaced by the nine-digit long serial numbers from the production licenses used for the upgrade.
