...
[.lead]
ONTAP Select is a software-only version of ONTAP that you can deploy as a virtual machine on a hypervisor host. It complements the suite of mainstream AFF and FAS ONTAP offerings as well as other software-only options such as Cloud Volumes ONTAP.
...
ONTAP Select is integrated into cloud management frameworks such as VMware vSphere and Kernel-based Virtual Machine (KVM). This helps accelerate deployment of new projects such as file services, home directories, software development environments, and application testing. 
...
In a commodity world where data is fragmented across silos of direct-attached storage (DAS), data access and management have become more complex problems. Software-defined storage (SDS) has emerged as an important part of the SDI landscape to address these and other issues.
...
An ONTAP Select cluster is composed of one, two, four, six, eight, ten, or twelve nodes. Each cluster node is deployed as a separate virtual machine and runs a specially-designed version of the ONTAP 9 software.
...
ONTAP Select supports FlexCache cache volumes with write-back. The overall ONTAP Select instance performance is comparable to an entry-level ONTAP system.

FlexCache cache volumes with write-back have the following requirements with ONTAP Select:

* Use ONTAP Select on a medium (8-core) or large (16-core) instance.
* Use NVMe-backed solid state drives. SAS-backed storage has a lower performance compared to NVMe-backed storage for ONTAP Select.
* Test ONTAP Select for FlexCache write-back before starting the deployment. FlexCache write-back is not recommended for a large number of small files.

ONTAP Select does not support FlexCache cache volumes with write-back on a small instance (4-core). Additionally, ONTAP Select does not support FlexCache origin volumes with write-back on small, medium, or large instances.

link:https://docs.netapp.com/us-en/ontap/flexcache-writeback/flexcache-write-back-guidelines.html[Learn about ONTAP FlexCache write-back^]
...
* Consistency groups in Asynchronous SnapMirror
* Fibre Channel
+
...
* link:reference_lic_ontap_features.html#licensed-ontap-features[Learn about ONTAP features licensed by default]
* link:concept_lic_evaluation.html[Learn about ONTAP Select license options]

// 2026 JUL 15, ONTAPDOC-4067
// 2026 JUN 15, ONTAPDOC-3941
// 2026 MAR 30, ONTAPDOC-3764
// 2026 MAR 20, ONTAPDOC-3153
