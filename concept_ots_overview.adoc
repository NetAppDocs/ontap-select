...
[NOTE]
====
ONTAP Select does not support:

* FlexCache cache volumes with write-back on small instances (4-core).
* FlexCache origin volumes with write-back on small, medium (8-core), and large (16-core) instances.

====

FlexCache cache volumes with write-back have the following requirements with ONTAP Select:

* Use an ONTAP Select medium or large instance.
