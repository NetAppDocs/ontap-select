...
FlexCache cache volume for write-back is supported with ONTAP Select. The overall ONTAP Select instance performance is comparable to a low-end ONTAP system.

NOTE: FlexCache origin with write-back is not supported with ONTAP Select.

A FlexCache cache volume for write-back with ONTAP Select configuration has the following requirements:

* Use an ONTAP Select medium (8-core) or large (16-core) instance. Small ONTAP Select instances (4-core) are not supported.
* Use NVMe backed solid state drives. SAS backed storage is lower compared to NVMe for ONTAP Select.
...
For more information, refer to the link:https://docs.netapp.com/us-en/ontap/flexcache-writeback/flexcache-write-back-guidelines.html[ONTAP FlexCache write-back guidelines^].
