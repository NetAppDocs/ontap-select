...
* Use an ONTAP Select on medium (8-core) or large (16-core) instances.
...
ONTAP Select does not support FlexCache cache volumes with write-back on small instances (4-core). Additionally, ONTAP Select does not support FlexCache origin volumes with write-back on small, medium, and large instances.
...
* link:reference_lic_ontap_features.html#enabled-ontap-features[Learn about ONTAP features enabled by default]
