...
[.lead]
You can increase the cluster size of an exiting ONTAP Select cluster for ESXi and KVVM hypervisor hosts. For KVM hosts, you can increase the size from a six-node to an eight-node cluster and decrease the size from an eight-node to a six-node cluster. For ESXi hosts, you increase and decrease the cluster size in increments between six-node and twelve-node clusters. 
