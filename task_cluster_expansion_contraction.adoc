...
NOTE: To change the number of nodes in a cluster to a size that isn't supported by cluster expansion or contraction, you need to deploy a new multi-node cluster by using the link:task_deploy_cluster.html#[ONTAP Select Deploy administration utility] or the link:task_cli_deploy_cluster.html[Deploy utility CLI] instead of changing the size of the existing cluster.
