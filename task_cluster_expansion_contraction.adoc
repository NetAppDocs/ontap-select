...
[.lead]
You can increase the size of an existing ONTAP Select cluster from six to eight nodes and decrease the size from eight to six nodes. 
...
You intitiate the cluster expansion and contraction procedures from ONTAP Select Deploy using the CLI, API, or web interface.

== Hardware and storage considerations
The cluster expansion and contraction feature is supported on ESX and KVM hypervisor hosts.

[cols="2a,2a,2a" options="header"]
|===
|Hypervisor type | Supported versions | Supported beginning with ONTAP Select... 
| KVM
...
.About this task
In preparation for cluster expansion, new ESX and KVM hosts are added to the inventory and the details of the new nodes are assigned. Before starting the cluster expansion process, a network precheck verifies the selected internal network.
