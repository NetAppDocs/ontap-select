...
---
= Host modify command for ONTAP Select Deploy
:hardbreaks:
:nofooter:
:icons: font
:linkattrs:
...
a| The username that has access to this host. This is required only if the host is not managed by a management server (that is, an ESXi host managed by a vCenter).
