...
---
= ONTAP Select HA RSM and mirrored aggregates
:hardbreaks:
:nofooter:
:icons: font
:linkattrs:
...
A two-node cluster with a mediator can span two data centers. For more information, refer to the section link:reference_plan_best_practices.html#two-node-stretched-ha-metrocluster-sds-best-practices[Two-node stretched HA (MetroCluster SDS) best practices].
