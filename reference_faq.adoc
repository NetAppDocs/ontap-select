...
All other errors are handled by ONTAP HA in the case of a two-node or four-node cluster. If the hypervisor server needs to be replaced and the ONTAP Select cluster needs to be reconstituted with a new server, refer to <<restore-ontap-select-cluster,Can I recover an ONTAP Select node?>>.

[[restore-ontap-select-cluster]]
