...
Modifying the networking details of the Deploy instance after it's running in an environment is not supported. For more information, see the KB article link:https://kb.netapp.com/onprem/ontap/ONTAP_Select/Modifying_DNS_configuration_of_ONTAP_Deploy_instance[Modifying DNS configuration of ONTAP Deploy instance^].
