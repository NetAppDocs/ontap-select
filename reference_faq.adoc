...
[NOTE]
Beginning with ONTAP Select 9.10.1 the KVM hypervisor is no longer supported when creating a new cluster. Further, beginning with 9.11.1 you can no longer manage existing ONTAP Select clusters on KVM. The questions involving KVM are applicable only to earlier releases that support KVM.
