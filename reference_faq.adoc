...
[NOTE]
Beginning with ONTAP Select 9.10.1 you can no longer deploy a new cluster on the KVM hypervisor. Now with ONTAP Select 9.11.1 and later releases, all manageability functionality is no longer available for existing KVM clusters and hosts except the take offline and delete functions. The questions involving KVM are applicable only to earlier releases that support KVM.
