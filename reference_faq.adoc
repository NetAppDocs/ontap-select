...
* For ONTAP Select (KVM) and ONTAP Select (ESXi) using software RAID, it is not possible to revert to a prior version that does not support software RAID. Furthermore, a new installation of ONTAP Select 9.5 or later on ESXi uses VMXNET3 network drivers and, when possible, the vNVMe driver. These new installations cannot be reverted to prior versions of ONTAP Select.
...
.Can a single ONTAP Select Deploy instance create clusters on both ESXi and KVM?

Yes. ONTAP Select Deploy can be installed on either KVM or ESXi, and both installations can create ONTAP Select clusters on either hypervisor.

.Is vCenter required for ONTAP Select on ESXi?

If the ESXi hosts are properly licensed, then there is no need for the ESXi hosts to be managed by a vCenter Server. However, if the hosts are managed by a vCenter server, then you must configure ONTAP Select Deploy to use that vCenter Server. In other words, you cannot configure ESXi hosts as standalone in ONTAP Select Deploy if they are being actively managed by a vCenter Server. Note that the ONTAP Select Deploy VM relies on vCenter to track all ONTAP Select VM migrations between ESXi hosts due to a vMotion or VMware HA event.
...
ONTAP Select Deploy and ONTAP Select for ESXi support the configuration of an ONTAP Select single-node cluster using either a vSAN or an external array type of datastore for its storage pool.
...
.Can you have a host with a single 10GE port run ONTAP Select, and is it available for both ESXi and KVM?

You can use a single 10GE port to connect to the external network. However, NetApp recommends that you use this only in constrained small form-factor environments. This is supported with both ESXi and KVM.
...
ONTAP Select Deploy uses the VMware VIX API to communicate with the vCenter, ESXi host, or both. The VMware documentation states that the initial connection to either a vCenter Server or an ESXi host is done using HTTPS/SOAP on TCP port 443. This is the port for secure HTTP over TLS/SSL. Secondly, a connection to the ESXi host is opened on a socket on TCP port 902. Data going over this connection is encrypted with SSL. Additionally, ONTAP Select Deploy issues a `PING` command to verify that there is an ESXi host responding at the IP address you specified.
...
For multi-node clusters, the internal network must be fully opened (no NAT or firewalls).
...
MetroCluster SDS provides a synchronous replication solution and falls under NetApp MetroCluster solutions. However, the key differences is the connectivity type (only IP networks are supported rather than FC and IP).
...
Yes, Software RAID is supported with SSD media on both KVM and ESXi.
...
Storage vMotion is supported for all configurations, including single-node and multi-node ONTAP Select clusters and the ONTAP Select Deploy VM. Storage vMotion can be used to migrate the ONTAP Select or the ONTAP Select Deploy VM between different VMFS versions (VMFS 5 to VMFS 6 for example), but it is not restricted to this use case. The best practice is to shut down the VM before initiating a Storage vMotion operation. ONTAP Select Deploy must issue the following operation after the storage vMotion operation is completed:

----
cluster refresh
----

NOTE: A storage vMotion operation between different types of datastores is not supported. In other words, storage vMotion operations between NFS-type datastores and VMFS datastores are not supported. In general, storage vMotion operations between external datastores and DAS datastores are not supported.

.Can the HA traffic between ONTAP Select nodes run over a different vSwitch and/or segregated physical ports and/or using point-to-point IP cables between ESXi hosts?
